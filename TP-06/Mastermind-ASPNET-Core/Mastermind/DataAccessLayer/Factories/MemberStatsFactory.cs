...
namespace Mastermind.DataAccessLayer.Factories
{
    ...
    {
        private MemberStats CreateFromReader(MySqlDataReader reader) => new(
        ...
        public void CreateForMember(int memberId)
        {
            ...
            cmd.CommandText = @"
                ...
                (MemberId,GamesWon,GamesLost,BestPerformance)
                VALUES
                (@MemberId,0,0,NULL)";
        ...
        public void RegisterWin(int memberId, int attemptsUsed)
        {
            using MySqlConnection cnn = new(DAL.ConnectionString);
            cnn.Open();
            ...
            // Update wins and keep best performance if current attempt is better
            cmd.CommandText = @"
            ...
            cmd.Parameters.AddWithValue("@AttemptsUsed", attemptsUsed);
            cmd.Parameters.AddWithValue("@MemberId", memberId);
